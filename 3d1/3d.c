...
// 3d v2 만들어서 깊이 double로 저장하고 품질 변화 확인하기
...
void reset_depth_table(float *depth_table)
{
    for (int x=0; x<SCREEN_X; x++)
    {
        for (int y=0; y<SCREEN_Y; y++)
...
float get_depth(Point A, Point B, Point C, Point A3, Point B3, Point C3, int x, int y)
{
    ...
    float i = vA_B_.x*vA_C_.y - vA_B_.y*vA_C_.x;
    float j = x-A.x;
    float k = y-A.y;
...
void draw_triangle(Color *screen, float *depth_table, Point A3, Point B3, Point C3, Color clr, Point light)
{
...
void draw_screen(Color *screen, float *depth_table, int triangle_num, Triangle model_data[], FILE *movement_file, int frame)
{
    reset_screen(screen);
...
int main()
{
    ...
    float *depth_table = malloc(sizeof(float) * SCREEN_X * SCREEN_Y);
    short x = SCREEN_X;
    short y = SCREEN_Y;
    ...
    if (model_file == NULL || movement_file == NULL || screen_file == NULL)
    {
